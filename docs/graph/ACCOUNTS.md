...
## Contas reserva e rotação por cota

No Matrix, abra `[A] Contas` e depois `[7] Contas reserva Codex`. Cada conta
reserva recebe um `CODEX_HOME` próprio. O login é feito pelo comando nativo do
Codex, e o cadastro local guarda apenas apelido, identificador e caminho do
perfil; tokens permanecem sob o armazenamento do Codex.

Quando uma chamada Codex retorna uma mensagem explícita de cota, o grafo
registra o evento, escolhe o próximo perfil autenticado e repete somente a
operação que não produziu resultado. A conta preferencial é atualizada para
evitar testar continuamente um perfil já sem cota. Falha, timeout ou resultado
ambíguo não é tratado como cota e não recebe reenvio automático.

Para tarefas textuais, e para geração de imagem quando o `agy` oferecer uma
capacidade nativa compatível, o próximo fallback é o Antigravity CLI. Revisão
visual só usa Antigravity se o CLI declarar suporte a anexos de imagem; sem
essa capacidade o LangGraph permanece no gate `IMAGE_HUMAN_REVIEW`.
