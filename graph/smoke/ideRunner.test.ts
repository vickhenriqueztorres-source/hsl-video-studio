...
`);
  ...
  try {
    ...
    process.env.LOCALAPPDATA = path.join(fixture, 'no-installed-agy');
  ...
  } finally { process.env.PATH = priorPath; process.env.LOCALAPPDATA = priorLocalAppData; }
  ...
  const quotaHang = await runProcess({ command: process.execPath, prefix: [] },
    ['-e', "console.error('usage limit reached'); setInterval(() => {}, 1000)"], REPO_ROOT, 5000,
    undefined, undefined, undefined, /usage limit/i);
  assert.equal(quotaHang.timedOut, false, 'explicit quota output must abort before the normal timeout');
  assert.match(unavailableReason(quotaHang) ?? '', /cota/);
