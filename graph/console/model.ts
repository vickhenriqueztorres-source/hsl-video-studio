...
export async function overview(episode:string,root=REPO_ROOT){
  ...
  try{
    ...
    return{episode,title:plan?.episodeTitle??episode,subtitle:plan?.subtitle??'',thesis:plan?.thesis??'',threadId:configFor(episode).configurable.thread_id,status:live.status,updatedAt:live.updatedAt,next,interrupts,nodes,phases:PHASES,progress:live.percent,live,
      agents:AGENTS.map(a=>({...a,status:stateOf(nodes,a.nodes),progress:Math.round(a.nodes.reduce((sum,id)=>sum+(nodes.find(n=>n.id===id)?.progress??0),0)/a.nodes.length)})),klingBudget:{totalTakes:v.mediaPlan?.totalTakes??(v.videoTakes??[]).length,approvedLimit:v.klingAuthorization?.limit??0,approvedAt:v.klingAuthorization?.approvedAt??null,policy:v.mediaPlan?.policy??null,planHash:v.mediaPlan?.hash??null},
