...
export async function runAntigravity(prepared: PreparedTask): Promise<DriverResult> {
  ...
  // Antigravity's current `-i` means prompt-interactive, unlike Codex's
  // image attachment flag. Only an explicit --image declaration is safe.
  if (prepared.task.imageFiles?.length && !flags.includes('--image')) {
    return { skipped: true, reason: 'Antigravity CLI não aceita imagens anexadas neste modo headless; revisão visual requer um provedor multimodal compatível.' };
  }
  ...
  if (prepared.task.imageFiles?.length && flags.includes('--image')) for (const image of prepared.task.imageFiles) args.push('--image', image);
