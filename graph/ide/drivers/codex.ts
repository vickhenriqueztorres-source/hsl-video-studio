...
export async function runCodex(prepared: PreparedTask): Promise<DriverResult> {
  const env=prepared.task.codexHome?{...process.env,CODEX_HOME:prepared.task.codexHome}:process.env;
  const cli = findCli('codex',env);
  if (!cli) return { skipped: true, reason: 'codex nao encontrado no PATH nem na instalacao do Codex Desktop.' };
  const help = await runProcess(cli, ['exec', '--help'], prepared.repoRoot, 30_000, prepared.logPath,undefined,env);
  ...
  args.push('-');
  const result = await runProcess(cli, args, prepared.repoRoot, prepared.timeoutMs, prepared.logPath,input,env,/usage limit|quota exceeded|insufficient\.quota|credits? exhausted|reached your.*limit/i);
  if (!hasOutput) {
