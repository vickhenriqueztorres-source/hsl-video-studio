...
export const fireflyVideos = (c: Context): NodeFn => async (s, config) => {
  assertMediaPlan(s);
  const beats = s.scenePlan!.beats.filter(b => b.visualMode === 'firefly_video'&&b.mediaProvider==='local-ffmpeg');
  if(!beats.length)return{__status:'skipped'};
  ...
  if (!before.every(Boolean) || !readJson(guide)) {
    try { await c.deps.videos(s.episodeId, beats); }
    catch (e) { if (attempts < 2) throw e; error = e instanceof Error ? e.message : String(e); }
  }
  const videos: AssetResult[] = beats.map((b, i) => ({ beatId: b.beatId, path: locations[i], provider:'local-ffmpeg', status: valid(i) ? (before[i] ? 'skipped' : 'ok') : 'failed',
