...
interface ReviewInput {beatId:string;prompt:string;imagePath:string;imageHash:string;continuityRefs?:string[];narration?:string;episodeBrief?:unknown}
...
export const imageReviewWait:NodeFn=s=>{
  if(s.options.graph.mediaMode==='legacy')return{__status:'skipped'};
  // A worker killed during image_review_prepare can leave a valid checkpoint
  // whose next task is the wait node but whose review payload was never
  // committed. Let the graph route back to preparation instead of
