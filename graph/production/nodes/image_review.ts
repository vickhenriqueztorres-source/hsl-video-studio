...
export const imageReviewPrepare=(c:Context):NodeFn=>async s=>{
  ...
  const run=await c.deps.ide({threadId:s.episodeId,node:'image-review',attempt:round,provider:'codex',ioMode:'stdout',readOnly:true,maxAttempts:2,imageFiles:inputs.map(x=>x.imagePath),
    promptTemplate:'graph/prompts/image-review.md',schemaPath:'graph/prompts/image-review.schema.json',vars:{images:JSON.stringify(inputs),threshold:String(s.options.graph.imageReviewThreshold)}},{repoRoot:c.root});
  ...
  const failed=items.filter(x=>x.score<s.options.graph.imageReviewThreshold||x.hasText);
...
export const imageReviewWait:NodeFn=s=>{
  if(s.options.graph.mediaMode==='legacy')return{__status:'skipped'};const r=s.imageReview!;const passed=r.items.length>0&&r.items.every(x=>x.score>=s.options.graph.imageReviewThreshold&&!x.hasText);
  if(passed||s.imageHumanApproved)return{};
  if(!r.skipped&&r.round<2){interrupt({kind:'IMAGE_QUEUE',queuePath:s.imageQueuePath,pendingCount:r.items.filter(x=>x.score<s.options.graph.imageReviewThreshold||x.hasText).length,skill:'hsl-image-worker',reviewIssues:r.items});return{};}
...
export const routeImageReview=(s:any)=>{const passed=s.imageReview?.items?.length&&s.imageReview.items.every((x:any)=>x.score>=s.options.graph.imageReviewThreshold&&!x.hasText);return passed||s.imageHumanApproved?'join_frames':'image_generate_wait';};
