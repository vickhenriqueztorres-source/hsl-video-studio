...
export const imageGeneratePrepare=(c:Context):NodeFn=>s=>{
  ...
  const queue:ImageQueue={episodeId:s.episodeId,threadId:threadId(s.episodeId),generator:'codex-imagegen',spec:{aspect:'16:9',minWidth:1920,format:'png',noText:true},
    items:all.map((x:any)=>{const p=old.get(x.beatId);return{beatId:x.beatId,promptPath:x.promptPath,outputPath:x.expectedPath,status:p?.status??'pending',attempts:p?.attempts??0,...(p?.lastError?{lastError:p.lastError}:{}),...(p?.generatedBy?{generatedBy:p.generatedBy}:{})};}),
...
export const imageGenerateWait=(c:Context):NodeFn=>s=>{
  ...
  for(let round=1;round<=3;round++){
    ...
    for(const item of queue.items){const v=validateImage(item.outputPath);if(!v.ok){item.status='pending';item.lastError=v.error;bad.push({beatId:item.beatId,error:v.error!});continue;}item.status='done';item.lastError=undefined;item.generatedBy='codex-imagegen';frames.push({beatId:item.beatId,path:item.outputPath,status:'ok',attempts:item.attempts});}
    writeJson(queuePath,queue);
