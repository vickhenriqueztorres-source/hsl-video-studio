...
export const paths = (c: Context, s: Pick<State, 'episodeId'> & Partial<Pick<State, 'options'>>) => {
  ...
  return { run, audit: path.join(run, 'graph'), manifest: path.join(run, 'run-manifest.json'),
    ...
    narrationSource: path.join(run, 'audio', 'narration-source.mp3'),
    narration: path.join(run, 'audio', 'narration-master.wav'),
    legacyNarration: path.join(run, 'audio', 'narration.mp3'),
    narrationQa: path.join(run, 'audio', 'narration-audio-qa.json'),
    narrationReceipt: path.join(run, 'audio', 'narration-receipt.json'),
    publicNarration: path.join(c.root, 'public', 'runs', s.episodeId, 'audio', 'narration-master.wav') };
};
...
export function endStage(c: Context, s: State, id: StageName, metrics?: Record<string, unknown>, artifacts?: Partial<RunManifestData['artifacts']>) {
  ...
  // A graph checkpoint may precede the run manifest (for example after a
  // recovered or derived run). Reconcile the manifest with the committed
  // LangGraph state instead of leaving a stage permanently PENDING.
  if (!done || id === 'STAGE_01_SCENE_PLAN') m.completeStage(id, metrics);
