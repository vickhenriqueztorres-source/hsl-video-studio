...
export function takeRecipe(s:State,t:Pick<VideoTake,'beatId'|'takeIndex'|'dependsOnTake'|'revision'>):string {
  ...
  return digest({plan:media.hash,beat:t.beatId,take:t.takeIndex,revision:t.revision??0,prompt:p.videoPrompt,rootImage:hashFile(image.path),profile:['Kling 2.5 Turbo',5,1920,1080,false],predecessor:t.dependsOnTake});
}
