...
export function takeCount(durationSeconds: number): number {
  ...
  // Full timeline coverage; 5.01s cannot silently freeze a five-second take.
  return Math.ceil(durationSeconds / TAKE_SECONDS);
}
...
/** Review chained takes as continuations; completed actions must not be repeated or reversed. */
export function qaPromptForTake(prompt:VisualPrompt,take:Pick<VideoTake,'dependsOnTake'>):string {
  if(!take.dependsOnTake)return prompt.videoPrompt;
  return `Continuation take from the approved predecessor frame. The predecessor may already have completed the main action described below. Preserve the physical state visible in the reference, do not repeat or reverse a completed action, and allow only a natural continuation or hold of that terminal state. Core shot context: ${prompt.videoPrompt}`;
}
