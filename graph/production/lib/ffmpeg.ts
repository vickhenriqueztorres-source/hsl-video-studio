...
export function createFfmpeg(root: string) {
  ...
  return {
    ...
    muxFinalWithSfx: async (visualPath: string, musicPath: string, narrationPath: string, sfxPath: string, outPath: string, bitrate: string) => requireSuccess(await run(['-y','-hide_banner','-loglevel','error','-i',visualPath,'-stream_loop','-1','-i',musicPath,'-i',narrationPath,'-i',sfxPath,'-filter_complex','[1:a]volume=0.04[bg];[2:a]asplit=2[voice][side];[bg][side]sidechaincompress=threshold=0.125:ratio=8:attack=20:release=500[ducked];[3:a]volume=1.0[sfx];[ducked][voice][sfx]amix=inputs=3:duration=first:normalize=0,alimiter=limit=0.95[aout]','-map','0:v:0','-map','[aout]','-c:v','copy','-c:a','aac','-b:a',bitrate,'-shortest',outPath]),'FFMPEG_MUX_SFX_FATAL'),
  };
}
