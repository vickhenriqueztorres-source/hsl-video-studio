...
export function normalizePlanDuration(plan: HslLongFormProjectPlan, targetMinutes: number): HslLongFormProjectPlan {
  ...
  if (targetFrames === sourceFrames && plan.totalFrames === sourceFrames && plan.totalBeatsCount === plan.beats.length) {
    return plan.beats.every(beat => beat.sourceBeatId) ? plan
      : { ...plan, beats: plan.beats.map(beat => ({ ...beat, sourceBeatId: beat.sourceBeatId ?? beat.beatId })) };
  }
