...
if(args.includes('--help')){console.log('--feed-guide --run --probe-session --requeue-unstarted-infra-job --recover-result-ready-job');process.exit(0);}
...
async function main(){
  ...
  // Isolate the fake agent from real browser process discovery on this host.
  const spawnTool=proc.spawnTool;
  (proc as any).spawnTool=(command:string,args:string[],options:proc.ToolOptions)=>command==='powershell.exe'||command==='ps'
    ?Promise.resolve({exitCode:0,stdout:command==='powershell.exe'?'false':'',stderr:'',timedOut:false,durationMs:0})
  ...
  try{
    ...
    assert.equal(fs.readFileSync(path.join(ok,'calls.log'),'utf8'),'feed\nrun\n');assert.equal(JSON.parse(fs.readFileSync(path.join(ok,'dispatch-receipt.json'),'utf8')).phase,'transport_complete');
  ...
  }finally{(proc as any).spawnTool=spawnTool;if(previous===undefined)delete process.env.HSL_ALLOW_PAID_FIREFLY_DISPATCH;else process.env.HSL_ALLOW_PAID_FIREFLY_DISPATCH=previous;}
}
