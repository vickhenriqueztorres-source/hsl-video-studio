...
async function main() {
  ...
  try {
    ...
    assert.deepEqual(done.values.mediaPlan?.localMotionBeatIds,['SCENE_002']);
    assert.deepEqual(done.values.mediaPlan?.fireflyBeatIds,[]);
    assert.deepEqual(done.values.mediaPlan?.stillBeatIds,['SCENE_001','SCENE_003']);
    assert.equal(done.values.scenePlan?.beats[2].visualMode,'generated_image_35mm','diagram video is correctly projected to a still');
    assert.equal(done.values.videos.length,1);
    assert.equal(done.values.videos[0].provider,'local-ffmpeg');
    ...
    const validators=new Set(['media_plan_prepare','media_plan_validate','archive_compliance']);
    assert.ok(again.values.timings.slice(timingCount).every((t: Timing) => validators.has(t.node) ? t.status==='ok' : t.status === 'skipped'), 'only deterministic media planning and validation rerun; cached production stages remain skipped');
