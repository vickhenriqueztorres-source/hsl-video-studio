...
import { configFor, executeProduction } from '../runner';
...
import { renderDurationSeconds, renderFrameRanges, renderPropsMatchPlan } from '../lib/remotion';
...
assert.deepEqual(renderFrameRanges({options:initialState({graph:{mediaMode:'legacy'}}).options!,scenePlan:threeMinutePlan}),[[0,4499],[4500,5399]]);
assert.deepEqual(renderFrameRanges({options:initialState({graph:{testRender:true}}).options!,scenePlan:threeMinutePlan}),[[0,299]]);
assert.equal(renderDurationSeconds({options:threeMinuteState.options!,scenePlan:threeMinutePlan}),180);
assert.equal(renderPropsMatchPlan({scenePlan:threeMinutePlan},{...threeMinutePlan,assetBaseUrl:'http://old'}),true);
assert.equal(renderPropsMatchPlan({scenePlan:threeMinutePlan},{...threeMinutePlan,totalFrames:18000}),false);
...
function setup(name:string,opts:{missingTake?:number;lowReview?:boolean;reviewUnavailable?:boolean;reviewAlwaysLow?:boolean;imageLow?:boolean;codexSkipped?:boolean;sessionInvalid?:boolean;unresolved?:boolean;busy?:boolean;authMissing?:boolean}={}){
  ...
  const plan=f.deps.plan!({} as any),selectedBeats=plan.beats.slice(0,2).map((b:any,i:number)=>({
    ...b, infographicArchetype:undefined,
    visualMode:i===0?'generated_image_35mm':'firefly_video',
    promptSubject:i===0?'A quiet airport apron at dawn':'A fuel hose coupling beside an aircraft wing',
    motionIntent:i===0?'none':'camera',
    motionReason:i===0?'Static establishing image':'Reveal the physical coupling with a continuous camera move',
    durationSeconds:i===0?5:7,durationFrames:i===0?150:210,
  }));
  const selected={...plan,beats:selectedBeats,totalBeatsCount:selectedBeats.length,totalFrames:selectedBeats.reduce((n:number,b:any)=>n+b.durationFrames,0),totalDurationSeconds:selectedBeats.reduce((n:number,b:any)=>n+b.durationSeconds,0)};
  ...
  const deps:any={...f.deps,
    ...
    ide:async(task:any)=>{
      ...
      if(task.node.startsWith('visual-prompts-review-')){
      ...
      if(task.node.startsWith('visual-prompts-repair-')){
        const affected=JSON.parse(task.vars.affectedScenePlan);
        return{prepared:{} as any,headlessResult:{provider:task.provider,ok:true,outputPath:'mock',durationMs:1,output:prompts(affected)}};
      }
    ...
    runFireflyTake:async(_e:any,runtime:string,guidePath:string,_log:string,auth:FireflyAuthorization)=>{
      for(const key of ['authorizationId','planHash','operationId','recipeHash'] as const)assert.ok(auth?.[key],`dispatch requires ${key}`);
      if(opts.busy)throw new Error('FIREFLY_PROFILE_IN_USE:D:\\HSL-FIREFLY-PROFILE');
      runCalls++;
      const g=JSON.parse(fs.readFileSync(guidePath,'utf8')),item=g.items[0],produced=path.join(runtime,'saida',item.name+'.mp4');
      const hash=(bytes:Buffer|string)=>createHash('sha256').update(bytes).digest('hex');
      const inputFrameHash=hash(fs.readFileSync(item.image));
      const stagedGuide={...g,items:[{...item,image:item.name+path.extname(item.image).toLowerCase()}]};
      media(produced,false,5);
      const outputHash=hash(fs.readFileSync(produced));
      fs.writeFileSync(path.join(runtime,'dispatch-receipt.json'),JSON.stringify({
        schema:'hsl.kling-dispatch.v2',authorization:{...auth},phase:'transport_complete',
        name:item.name,guideHash:hash(JSON.stringify({guide:stagedGuide,inputFrameHash})),inputFrameHash,
        paidDispatchPossible:true,outputPath:produced,outputHash,
        updatedAt:new Date().toISOString(),
      },null,2));
      if(runCalls===opts.missingTake)fs.unlinkSync(produced); // Loss after durable transport completion.
      return{feed:{exitCode:0},run:{exitCode:0},outputHash};
    },
    ...
    reviewFireflyTake:async()=>({passed:true,issues:[]}),
    fitSceneVideo:async(_input:string,out:string,frames:number)=>{media(out,false,frames/30);},
    ...
    renderChunk:async(r:string,e:string,index:number,range:[number,number])=>{media(path.join(r,'out',`temp_p${index+1}_${e.toLowerCase()}.mp4`),false,(range[1]-range[0]+1)/30);return{exitCode:0,stdout:'',stderr:'',timedOut:false,durationMs:1};},
    muxFinalWithSfx:async(v:string,_m:string,_n:string,_s:string,out:string)=>media(out,false,JSON.parse(fs.readFileSync(v,'utf8').split('}')[0]+'}').durationSeconds),
  };
...
async function main(){
  ...
  const inventoryRoot=path.join(base,'frame-inventory'),inventoryEpisode='FRAME_RECOVERY',inventoryImage=path.join(inventoryRoot,'runs',inventoryEpisode,'images','SCENE_001','SCENE_001.png');
  fs.mkdirSync(path.dirname(inventoryImage),{recursive:true});fs.copyFileSync(imageFixture,inventoryImage);
  const inventoryQueue=path.join(inventoryRoot,'runs',inventoryEpisode,'images','QUEUE.json');
  fs.writeFileSync(inventoryQueue,JSON.stringify({episodeId:inventoryEpisode,threadId:inventoryEpisode+'@v2',generator:'codex-imagegen',spec:{aspect:'16:9',minWidth:1920,format:'png',noText:true},items:[{beatId:'SCENE_001',promptPath:'prompt',outputPath:inventoryImage,status:'done',attempts:1}],resumeCommand:'resume'}));
  const restored=resolveValidatedFrames({root:inventoryRoot,deps:{} as any},{...initialState({episodeId:inventoryEpisode}),imageQueuePath:inventoryQueue,frames:[]} as any,['SCENE_001']);
  assert.equal(restored[0].path,inventoryImage);assert.ok(fs.existsSync(path.join(inventoryRoot,'runs',inventoryEpisode,'frames','SCENE_001.png')));
  const pending=JSON.parse(fs.readFileSync(inventoryQueue,'utf8'));pending.items[0].status='pending';fs.writeFileSync(inventoryQueue,JSON.stringify(pending));
  fs.rmSync(path.join(inventoryRoot,'runs',inventoryEpisode,'frames','SCENE_001.png'));fs.rmSync(path.join(inventoryRoot,'public','runs',inventoryEpisode,'frames','SCENE_001.png'));
  assert.throws(()=>resolveValidatedFrames({root:inventoryRoot,deps:{} as any},{...initialState({episodeId:inventoryEpisode}),imageQueuePath:inventoryQueue,frames:[]} as any,['SCENE_001']),/APPROVED_FRAME_MISSING/);
  console.log('PASS frame inventory: approved physical queue assets restore a cleared LangGraph frames channel');
  ...
  const normalizedCanonical=normalizePlanDuration(canonical,10);
  assert.deepEqual(normalizedCanonical.beats.map(b=>b.sourceBeatId),canonical.beats.map(b=>b.beatId));
  assert.equal(normalizePlanDuration(normalizedCanonical,10),normalizedCanonical,'normalized plans are reused without changing source lineage');
  assert.equal(six.totalBeatsCount,58);assert.equal(six.totalFrames,10800);assert.equal(six.beats.reduce((n,b)=>n+b.durationFrames,0),10800);assert.equal(six.acts.length,8);assert.equal(six.beats.at(-1)?.beatId,'SCENE_058');
  assert.deepEqual(renderFrameRanges({options:initialState({episodeId:'DURATION_CHECK',targetMinutes:6,graph:{mediaMode:'legacy'}}).options!,scenePlan:six}),[[0,4499],[4500,8999],[9000,10799]]);
  assert.equal(renderDurationSeconds({options:initialState({episodeId:'DURATION_CHECK',targetMinutes:6}).options!,scenePlan:six}),360);
  ...
  const happy=setup('REAL_HAPPY',{unresolved:true});try{media(path.join(happy.root,'out','test','REAL_HAPPY-2beats.mp4'),false,600);const s=await runNew(happy,'REAL_HAPPY');assert.deepEqual(s.next,['packaging_stage']);assert.equal(s.values.videoTakes.length,2);assert.equal(s.values.generationCount,2);assert.equal(s.values.sfxUnresolved.length,1);assert.equal(s.values.renderChunks.length,1);assert.equal(s.values.finalVideo?.durationSeconds,10);assert.ok(s.values.finalVideo?.outPath.includes(path.join('out','test')));console.log('PASS 1,6: real two-beat flow rejects a stale 600s master and replaces it with the exact 10s test render');}finally{happy.saver.db.close();}
  ...
  const recovery=setup('RECOVERY',{missingTake:2});try{
    let s=await runNew(recovery,'RECOVERY');
    const issue=s.tasks.flatMap(t=>t.interrupts)[0]?.value as any;
    assert.equal(issue.kind,'FIREFLY_RECOVERY');assert.equal(issue.retryPolicy,'manual-reconcile-no-auto-resubmit');
    const t=s.values.videoTakes.find((x:any)=>x.status==='dispatched')!;
    assert.ok(t);assert.equal(recovery.runCalls,2);
    const receipt=JSON.parse(fs.readFileSync(issue.receiptPath,'utf8'));
    assert.equal(receipt.schema,'hsl.kling-dispatch.v2');assert.equal(receipt.phase,'transport_complete');
    assert.equal(receipt.authorization.operationId,t.operationId);assert.equal(receipt.authorization.recipeHash,t.recipeHash);
    assert.equal(receipt.outputPath,path.join(path.dirname(issue.receiptPath),'saida',`${t.beatId}-take-${t.takeIndex}.mp4`));
    assert.ok(!fs.existsSync(receipt.outputPath));
    s=await executeProduction(recovery.graph,recovery.root,'RECOVERY',new Command({resume:{resumed:true}}));
    assert.equal((s.tasks.flatMap(t=>t.interrupts)[0]?.value as any).kind,'FIREFLY_RECOVERY');
    assert.equal(recovery.runCalls,2,'resuming without external output must not resend a paid job');
    media(receipt.outputPath,false,5);
    assert.equal(createHash('sha256').update(fs.readFileSync(receipt.outputPath)).digest('hex'),receipt.outputHash,'recovered output must match the completed transport receipt');
    s=await executeProduction(recovery.graph,recovery.root,'RECOVERY',new Command({resume:{resumed:true}}));
    assert.deepEqual(s.next,['packaging_stage']);assert.equal(recovery.runCalls,2,'receipt-linked output recovery must not resend');
    assert.equal(s.values.generationCount,2);assert.ok(s.values.videoTakes.every((take:any)=>['ok','skipped'].includes(take.status)));
    console.log('PASS 2: receipt-linked FIREFLY_RECOVERY resumes with zero paid resends');
  }finally{recovery.saver.db.close();}
  ...
  for(const [name,opts] of [['REVIEW_UNAVAILABLE',{reviewUnavailable:true}],['REVIEW_FAILED',{reviewAlwaysLow:true}]] as const){
    const x=setup(name,opts);try{const s=await runNew(x,name);const gate=s.tasks.flatMap(t=>t.interrupts)[0]?.value as any;assert.equal(gate.kind,'VISUAL_PROMPTS_HUMAN_REVIEW');assert.equal(x.imageRuns,0);assert.equal(x.runCalls,0);if(name==='REVIEW_FAILED')assert.equal(s.values.promptIteration,6);}finally{x.saver.db.close();}
  }
  console.log('PASS review failures: unavailable or six-times rejected prompts enter a recoverable human gate before images or Kling');
  const limit=setup('LIMIT');try{
    let s=await runNew(limit,'LIMIT',0);
    const interrupts=s.tasks.flatMap(t=>t.interrupts),gate=interrupts[0]?.value as any;
    assert.equal(gate.kind,'KLING_BUDGET');assert.equal(gate.requiredGenerations,2);assert.equal(limit.runCalls,0);
    assert.deepEqual(s.next,['kling_budget_wait']);
    s=await executeProduction(limit.graph,limit.root,'LIMIT',new Command({update:{options:{...s.values.options,graph:{...s.values.options.graph,keepLocalDeliverables:3}}},resume:{decision:'proceed',limit:2}}));
    assert.deepEqual(s.next,['packaging_stage']);assert.equal(s.values.generationCount,2);assert.equal(limit.runCalls,2);
    assert.equal(s.values.options.graph.maxGenerations,2);assert.equal(s.values.options.graph.keepLocalDeliverables,3);
    assert.equal(s.values.klingAuthorization?.source,'interactive');
    console.log('PASS 5: atomic CLI resume applies options and authorizes exactly two dispatches');
  }finally{limit.saver.db.close();}
