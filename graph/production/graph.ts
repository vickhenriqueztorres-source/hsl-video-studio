...
import { ProductionState, type State, type Update } from './state';
...
import { motionPlan } from './nodes/motion_plan';
...
import { narrationLock, motionDispatch, motionReviewWait, motionJoin, routeMotionDispatch } from './nodes/authored_motion';
...
import { selectScenePlan, selectImages, selectFirefly, selectMotion, selectAudio, selectCompliance } from './storage/selectors';
...
export function createProductionGraph(checkpointer?: BaseCheckpointSaver, overrides: Partial<Dependencies> = {}, root = REPO_ROOT, options: ProductionGraphOptions = {}) {
  ...
  // Bound fluent type expansion while retaining typed state and node names.
  const preparation = new StateGraph(ProductionState) as StateGraph<typeof ProductionState.spec, State, Update, NodeName>;
  preparation
  ...
  production
    ...
    .addNode('kling_budget_wait',timed(c,'kling_budget_wait',klingBudgetWait(c)))
    .addNode('firefly_recovery_wait',timed(c,'firefly_recovery_wait',fireflyRecoveryWait(c)))
    ...
    .addNode('narration_lock',timed(c,'narration_lock',narrationLock(c)))
    .addNode('motion_dispatch',timed(c,'motion_dispatch',motionDispatch(c)))
    .addNode('motion_review_wait',timed(c,'motion_review_wait',motionReviewWait))
    .addNode('motion_join',timed(c,'motion_join',motionJoin(c)))
    .addNode('archive_motion',timed(c,'archive_motion',archiveStage('motion',selectMotion)(c)),{retryPolicy})
    ...
    .addNode('gate_render_wait', timed(c,'gate_render_wait',gateRenderWait))
    ...
    .addNode('gate_publish_wait', timed(c,'gate_publish_wait',gatePublishWait))
  ...
  return production
    ...
    .addConditionalEdges('image_review_wait',s=>s.imageReview?routeImageReview(s):'image_review_prepare',['image_review_prepare','image_generate_run','archive_images']).addEdge('archive_images','join_frames')
    .addEdge('fan_out_frames', 'image_frames').addEdge('image_frames', 'join_frames')
    .addConditionalEdges('join_frames',s=>s.mediaPlan?.fireflyBeatIds.length?'firefly_guide':s.mediaPlan?.localMotionBeatIds.length?'fan_out_videos':'join_videos',['fan_out_videos','firefly_guide','join_videos'])
    .addEdge('firefly_guide','kling_budget_wait').addConditionalEdges('kling_budget_wait',routeKlingBudget,['firefly_session_prepare','firefly_dispatch','finalize'])
    ...
    .addEdge('join_videos','narration_stage').addEdge('narration_stage','narration_lock')
    .addConditionalEdges('narration_lock',s=>s.options.graph.motionMode==='authored'?'motion_dispatch':'sound_design',['motion_dispatch','sound_design'])
    .addConditionalEdges('motion_dispatch',routeMotionDispatch,['motion_dispatch','motion_review_wait','motion_join'])
    .addConditionalEdges('motion_review_wait',s=>s.productionStatus==='ABORTED'?'finalize':'motion_dispatch',['finalize','motion_dispatch'])
    .addEdge('motion_join','archive_motion').addEdge('archive_motion','sound_design').addEdge('sound_design','sfx_render').addEdge('sfx_render','archive_audio').addEdge('archive_audio','gatekeeper_stage')
