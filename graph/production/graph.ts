...
import { resolveMediaPolicy } from './lib/mediaPlan';
...
export function createProductionGraph(checkpointer?: BaseCheckpointSaver, overrides: Partial<Dependencies> = {}, root = REPO_ROOT, options: ProductionGraphOptions = {}) {
  ...
  return production
    .addEdge(START,'scene_plan').addEdge('scene_plan','media_plan_prepare').addEdge('media_plan_prepare','media_plan_validate').addEdge('media_plan_validate','env_check').addEdge('env_check','codex_auth_prepare').addEdge('codex_auth_prepare','codex_auth_wait').addConditionalEdges('codex_auth_wait',s=>s.codexAuth?.authenticated?'drive_auth_wait':'codex_auth_prepare',['drive_auth_wait','codex_auth_prepare']).addEdge('drive_auth_wait','archive_scene_plan')
    ...
    .addConditionalEdges('join_frames',s=>resolveMediaPolicy(s)==='firefly-hybrid'?'firefly_guide':'fan_out_videos',['fan_out_videos','firefly_guide'])
