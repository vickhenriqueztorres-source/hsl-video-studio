...
export const NODE_ORDER = ['scene_plan','env_check','visual_prompts_prepare','visual_prompts_wait','visual_prompts_review_prepare','visual_prompts_review_wait','image_generate_prepare','image_generate_wait','fan_out_frames','image_frames','join_frames','firefly_session_prepare','firefly_session_wait','firefly_guide','firefly_dispatch','firefly_intake_wait','firefly_finalize','fan_out_videos','firefly_videos','join_videos','narration_stage','sound_design','sfx_render','gatekeeper_stage','gate_render_wait','render_prepare','fan_out_render','render_chunk','stitch','pre_mux_gate','mux','packaging_stage','compliance_stage','gate_publish_wait','finalize'] as const;
...
export function createProductionGraph(checkpointer?: BaseCheckpointSaver, overrides: Partial<Dependencies> = {}, root = REPO_ROOT, options: ProductionGraphOptions = {}) {
  ...
  return new StateGraph(ProductionState)
    ...
    .addEdge('image_generate_prepare','image_generate_wait').addEdge('image_generate_wait','join_frames')
