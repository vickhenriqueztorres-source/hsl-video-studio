...
export interface ToolOptions {
  ...
  stdin?: string; onStdout?: (text: string) => void; onStderr?: (text: string) => void; abortOnOutput?: RegExp;
}
...
export async function spawnTool(cmd: string, argv: string[], opts: ToolOptions): Promise<ToolResult> {
  ...
  return new Promise(resolve => {
    ...
    // Provider quota output is terminal for this attempt. Force-close the
    // process tree so a CLI that keeps retrying its websocket cannot hold the
    // LangGraph worker until the normal generation timeout.
    const abortIfMatched=(s:string)=>{if(opts.abortOnOutput&&!settled&&opts.abortOnOutput.test(s))kill(true);};
    child.stdout.on('data', (s: string) => { stdout = (stdout + s).slice(-2_000_000); log('[stdout] ' + s); opts.onStdout?.(s); abortIfMatched(s); });
    child.stderr.on('data', (s: string) => { stderr = (stderr + s).slice(-128_000); log('[stderr] ' + s); opts.onStderr?.(s); abortIfMatched(s); });
...
export function spawnInteractiveTool(cmd:string,argv:string[],opts:{cwd:string;env?:NodeJS.ProcessEnv}):Promise<number>{
  const cli=resolveTool(cmd);
  return new Promise((resolve,reject)=>{const child=spawn(cli.command,[...cli.prefix,...argv],{cwd:opts.cwd,env:opts.env??process.env,shell:false,stdio:'inherit'});child.on('error',reject);child.on('close',code=>resolve(code??1));});
}
